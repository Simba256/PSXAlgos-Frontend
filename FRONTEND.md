...
**Run progress UI (2026-07-10):** while a job runs, `backtest-new-view.tsx` renders `RunProgress` (under the rail CTA on desktop, under the mobile CTA bar) — an animated bar + **date scrubber** fed by the watcher's `onProgress` callback (`progress_pct` from the 5s poll). The engine replays the market day by day, so percent-complete is mapped calendar-linearly onto the run's date range (frozen at submit in `runInfo`) and rendered as the live simulation date sweeping between the range endpoints (`Jul 10, 2024 — ⏵ Mar 12, 2025 — Jul 10, 2025`). Between polls a 1s ticker advances a smoothed percent at the average observed rate (capped at 99), so the date and bar keep moving instead of jumping every 5s. States: indeterminate sweep before the first tick ("starting engine…" → "loading market data…"), then the determinate fill with `X% · ~2m 10s left`. Remaining time is anchored at (pct, elapsed-at-pct) so it counts down between polls. The rail's "estimated scope" box also shows a pre-run `est. runtime ~N min`, extrapolated from a localStorage calibration (`psx.backtest.secsPerDecision`, 50/50-blended seconds-per-candidate-decision recorded on each finished run; hidden until the first run calibrates it).
