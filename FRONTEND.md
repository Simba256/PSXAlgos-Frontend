...
**Run progress UI (2026-07-10):** while a job runs, `backtest-new-view.tsx` renders `RunProgress` (under the rail CTA on desktop, under the mobile CTA bar) — an animated bar + **date scrubber** fed by the watcher's `onProgress` callback from the 5s poll. The backend reports two phases: `phase: "loading"` + `load_pct` while the engine batch-fetches market data (dominates wall time on wide universes — this used to show nothing, which read as "stuck"), then `phase: "simulating"` + `progress_pct` during the bar loop. Display states: (1) indeterminate sweep before any tick ("starting engine…"), (2) determinate **dimmed** fill at `load_pct` with "loading market data · X%", (3) full-strength fill driven by sim percent with the date scrubber — percent-complete mapped calendar-linearly onto the run's range (frozen at submit in `runInfo`) so the live simulation date sweeps between the endpoints (`Jul 10, 2024 — ⏵ Mar 12, 2025 — Jul 10, 2025`). Between polls a 1s ticker advances a smoothed sim percent at the rate observed **between the last two distinct polls** (delta rate — never pct ÷ total-elapsed, which the load phase would skew), capped at 99; the same rate drives the counting-down `X% · ~2m 10s left` ETA. The rail's "estimated scope" box also shows a pre-run `est. runtime ~N min`, extrapolated from a localStorage calibration (`psx.backtest.secsPerDecision`, 50/50-blended seconds-per-candidate-decision recorded on each finished run; hidden until the first run calibrates it).
