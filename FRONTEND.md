...
**Run progress UI (2026-07-10):** while a job runs, `backtest-new-view.tsx` renders `RunProgress` (under the rail CTA on desktop, under the mobile CTA bar) — a staged **journey bar** + **date scrubber**. The backend reports two phases through the 5s poll (`phase: "loading"` + `load_pct` during the batch data fetch, then `phase: "simulating"` + `progress_pct` during the bar loop), but real waiting exists before the first report, between the phases (multi-TF stamping), and after the last one (metrics + save) — so backend numbers drive the *middle* of the bar, not the whole of it: 0–20 time-based crawl ("starting engine…"), 20–50 `load_pct` ("loading market data · X%"), 50–55 time-based creep ("preparing indicators…"), 55–80 sim percent with the date scrubber, 80–99 time-based crawl ("computing metrics…" → "saving results…"). Time-based segments approach their ceiling exponentially (always moving, never past the next milestone); a monotonic guard stops the bar ever moving backwards; pre-sim segments render dimmed. The scrubber maps sim percent calendar-linearly onto the run's range (frozen at submit in `runInfo`): `Jul 10, 2024 — ⏵ Mar 12, 2025 — Jul 10, 2025`, pinned to the end date while finalizing. Between polls a 1s ticker advances a smoothed sim percent at the rate observed **between the last two distinct polls** (delta rate — never pct ÷ total-elapsed, which the load phase would skew), capped at 99; the same rate drives the counting-down `X% · ~2m 10s left` ETA. The rail's "estimated scope" box also shows a pre-run `est. runtime ~N min`, extrapolated from a localStorage calibration (`psx.backtest.secsPerDecision`, 50/50-blended seconds-per-candidate-decision recorded on each finished run; hidden until the first run calibrates it).
