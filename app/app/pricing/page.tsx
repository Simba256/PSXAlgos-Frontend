...
function tierPricing(
  ...
  const monthly = name === "Pro" ? 500 : 1500;
  if (billing === "monthly") {
    return { price: `PKR ${monthly.toLocaleString()}`, period: "/month" };
  }
  const yearlyPerMonth = Math.round(monthly * (1 - YEARLY_DISCOUNT));
  const total = yearlyPerMonth * 12;
  return {
    price: `PKR ${yearlyPerMonth.toLocaleString()}`,
    period: "/month",
    footnote: `PKR ${total.toLocaleString()} billed yearly · save 20%`,
  };
}
