          ...
          {scope === "by_sector" && (
            ...
            >
              <SectorPicker
                ...
                onAdd={(name) => {
                  if (!(filters.sectors ?? []).includes(name)) {
                    toggleSector(name);
                  }
                }}
...
// Searchable sector picker — mirrors SymbolPicker (text input + filtered
// dropdown + chip row below). The previous "show all 36 sector chips at
// once" rendering produced a wall of buttons that scrolled the modal; this
// matches the ticker picker's interaction so both sides of the universe
...
function SectorPicker({
  ...
  const matches = useMemo(() => {
    ...
    // Empty query: show first ~8 unselected so the dropdown is discoverable
    // on focus (sector names are 1–36, full list is short — unlike tickers).
    if (!q) {
      return available.filter((s) => !exclude.has(s)).slice(0, 8);
    }
    const starts: string[] = [];
    ...
    for (const s of available) {
      ...
      if (upper.startsWith(q)) starts.push(s);
      else if (upper.includes(q)) contains.push(s);
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", maxWidth: 360 }}>
      <input
        type="text"
        value={query}
        placeholder="search and add a sector"
        autoComplete="off"
        disabled={disabled}
        onChange={(e) => {
