...
- **X-alignment (depth-from-leaves columns, 2026-05-08)**: the layered layout in `layout.ts` originally placed nodes by depth-from-root (`columnLeftX(level)`), which made nested-group sub-leaves land in a column further from the root than their flat siblings — a tree with 3 leaves + 1 OR group of 2 sub-leaves rendered with a jagged input edge (3 leaves at level 1's X, 2 leaves at level 2's X, OR gate marooned in between). Refactor: new `buildColumnMap` walks the tree once and assigns each node a `col` = depth-from-leaves (leaves = 0, empty group = 1, populated group = 1 + max child col). `columnLeftX(col, rootCol)` anchors the root at `ROOT_GATE_X` while leaves all land at the same rightmost column regardless of nesting depth. Result: clean vertical leaf edge on both sides of the canvas; gates fan inward by their own subtree depth. For a depth-1 tree (no nested groups) the math collapses to identical pixel positions as before — the entry tree on strategy 22 is byte-for-byte unchanged.
