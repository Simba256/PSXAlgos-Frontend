...
Phase 4b spine-symmetry fix (2026-05-08): the entry root → Risk Defaults gap was 402px (`ROOT_TO_OUTPUT_GAP`) but the Risk Defaults → exit root gap was only 80px (`RISK_TO_EXIT_GAP`) — visually the entry tree floated far to the left of the hub while the exit tree hugged it on the right. First pass set both to 402px which symmetrized correctly but left a wide dead zone in the middle ("too much space in the centre, we should move them closer"); second pass collapsed both to a single `SPINE_GAP = 160` constant so the hub sits close to both trees while the bezier (80px control-point offset in `pushWire`) still has room for a gentle sweep instead of becoming a degenerate horizontal.
